...
object Main extends AccessTokenUtil with AppUtil with LastTradePriceUtil with OrderUtil with PositionUtil with QuoteUtil
    ...
    private val credentialConfig = ConfigFactory.load("credentials.conf")
    ...
    private def createRoute(heartbeatActor: ActorRef, webSocketActorRef: ActorRef): Route = {
        ...
        val webHome = sys.env.get("webHome") match {
            case Some(x) => x
            case _ =>
                println("ERROR: Need to run: export webHome=...")
                System.exit(-1)
                ""
        }
        ...
        path("tick") {
            ...
            complete(HttpEntity(ContentTypes.`text/plain(UTF-8)`,
                s"Send a tick at ${LocalDateTime.now format ISO_LOCAL_DATE_TIME}"))
        } ~
        path("m1") {
        ...
        path("clear-hash") {
            ...
            complete(HttpEntity(ContentTypes.`application/json`,
                s"""{"detail":"Sent M1.ClearHash to everybody at ${LocalDateTime.now format ISO_LOCAL_DATE_TIME}"}"""))
        } ~
