...
class SafariTextUpload {
    ...
    def beautifyHtmlFile() {
        ...
        def forPackt(document: Document): Unit = {
            object Utils {
                ...
                private def distance(e1: org.jsoup.nodes.Element, e2: org.jsoup.nodes.Element, d: Int): Int =
                    if (e1 == null || e2 == null) Int.MaxValue
                    else if (e1 == e2) d
                    else distance(e1.parent(), e2, d + 1)
            }
        ...
        Seq(
            ...
            wholeFile = """ data-mfp-src="/library/view/[a-z0-9-]+/[a-z0-9-]+/[_a-zA-Z0-9-]+\.[a-z]{3}"""".r
                    .replaceAllIn(wholeFile, "")
            ...
            wholeFile = """ class="calibre[0-9]*"""".r.replaceAllIn(wholeFile, "")
            wholeFile = """ alt=""""".r.replaceAllIn(wholeFile, "")
